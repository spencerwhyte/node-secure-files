module.decryptedDirectoryPath = basePath => `${basePath}/decrypted`;

module.encryptedDirectoryPath = basePath => `${basePath}/encrypted`;

module.decryptedProjectDirectoryPath = (basePath, projectId) => {
  const decryptedDirectory = this.decryptedDirectoryPath(basePath);
  return `${decryptedDirectory}/${projectId}`;
};

module.encryptedProjectDirectoryPath = (basePath, projectId) => {
  const encryptedDirectory = this.encryptedDirectoryPath(basePath);
  return `${encryptedDirectory}/${projectId}`;
};

module.decryptedFilePath = (basePath, projectId, configFileName) => {
...
module.encryptedFilePath = (basePath, projectId, configFileName) => {
